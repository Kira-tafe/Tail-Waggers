...
function userIsLoggedIn()//check if the login form has been submitted - Page 294
{
  if (isset($_POST['action']) and $_POST['action'] == 'login')
  {
    ...
    if (databaseContainsPoster($_POST['email'], $password))
    {
  ...
  {
    return databaseContainsPoster($_SESSION['email'], $_SESSION['password']);
  }
}

function databaseContainsPoster($email, $password)
{
  include '/tail-waggers-KG-main/includes/db.inc.php';
  ...
  {
    $error = 'Error searching for poster.';
...
function userHasRole($role)
{
  ...
  {
    $sql = "SELECT COUNT(*) FROM poster
        INNER JOIN posterrole ON poster.id = posterid
  ...
  {
    $error = 'Error searching for poster roles.';
    include 'error.html.php';
    exit();
  }
